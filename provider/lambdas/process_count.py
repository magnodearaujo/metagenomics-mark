...
def process_template(template, context=None):
    new_template = copy.deepcopy(template)
    status = 'success'

    print("Processing template to multiply resources based on 'Count' property...")

    print(f"template1 = {type(template)}: {template}")
    print(f"context1 = {type(context)}: {context}")
